...
The coarse grids are characterized by too much precipitation and too much melting and runoff, compared with RACMO. The SMB in coarse grids therefore have smaller errors than their individual components (Table~\ref{tbl:table3}), because large errors in the source and sink terms offset one another when added together. Such compensating errors highlights the importance of understanding the extent to which a model is getting the SMB right for the right reasons.
...
Lower troposphere temperature is not a strict proxy for melting; e.g., it may not capture microclimate effects as a result of a better representation of the low-elevation ablation zones. The Positive Degree-Day temperature based melt index \cite<PDD;>{B1984Z}, which accumulate the near-surface temperature in $^{\circ}C$ for days with temperature above freezing, is a more accurate proxy for melting. PDD is nonlinear in mean monthly temperature \cite{R1991P}.  We compute PDD from monthly mean 2-meter temperature using the method of \citeA{CG2005JG}, assuming a fixed monthly mean standard deviation of 3$^{\circ}$C and a degree-day factor of 5~mm~d$^{-1}$ $^{\circ}$C$^{-1}$.
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=145mm]{figs/OMEGA500FHS94.pdf}
\end{center}
