...
{\color{blue}{We would like to thank both reviewers for carefully reviewing our manuscript and providing detailed feedback. We have taken seriously the reviewers questions/concerns and hope they are satisfied with our responses. Some of the larger issues we addressed are as follows. We have added a section on computational costs, and added cost metrics to the main Table 1. A few paragraphs (and a figure) were added to understand the impact of albedo changes on the melt biases. It was not entirely clear whether we are running the ice sheet model (we are not), and so we tried to hammer that point home at the very top of the methods section. There were several definitions of the surface mass balance (SMB) throughout, and not all of them the same. This has been cleaned up for clarity, making clear that the internally computed SMB in CESM is a bit unconventional, and that we use the components of the internally computed SMB to construct a more complete SMB, which we evaluate throughout the paper. \newline

Note that a bug was found in the script that computes the components of the SMB, and therefore Figure 10 and 11, and Table 3 have been updated with the bug fix. This bug does not impact our analysis or conclusions.}}
...
{\color{blue}{We thank the reviewer for this point. We had not thought too much about albedo and so it's for the best that the reviewer brings this up. We have added a Figure and three paragraphs addressing the albedo changes in the simulations, which are significant. We have made a good faith effort to explore the precip-albedo mechanism that the reviewer describes. In general we don't find a great deal of evidence for the reviewers proposed mechanism. We believe this is largely because the positive interior precipitation biases are not offset by a negative precipitation bias at the coastal margins. We think it is more likely that the warmer near surface temperatures in the coarse grids are clawing back the snow fractions, and therefore the albedo.}}
