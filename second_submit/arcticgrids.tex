...
{\color{blue}{Another potential factor in explaining the differences across the various grids and dycores is the surface albedo. Bare ice has a lower albedo than snow, so it's reasonable to speculate whether the large melt biases in the coarse grid simulations are in part due to more frequent bare ice exposure. One logical pathway to more bare ice exposure is related to the precipitation and cloud biases. As the coarser models accumulate too much precipitation in the GrIS interior, the coastal ice margins receive too little snow, which can be more easily melted away to expose the underlying glacial ice. Figure~\ref{fig:alb} shows maps of summer albedo biases in the simulations, relative to the RACMO2.3p2 and CERES-EBAF datasets. The surface albedo is diagnosed from the incident and net shortwave radiative fluxes at the surface. The lat-lon and quasi-uniform grids all have anomalously low albedo around the coasts in the southeast and northwest regions of the GrIS, which coincides with regions of anomalously low cloud cover and precipitation. In the VR simulations, the low albedo biases are alleviated to a large extent.

In light of these albedo results, one may interpret Figure~\ref{fig:bias} differently. That there are larger melt biases for grid cells with only only partial ice cover, may have less to do with the partial ice cover and more to do with the enhanced bare ice exposure in the coarser grids. Further experimentation is required to tease out the relative contributions of these two processes.}}
